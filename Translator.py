...
# a helper function to print time elapsed and estimated time remaining given the current time and progress %.
import time
import math
...
def saveEncoderToPTL(encoder, path):
    ...
    optimized_encoder = optimize_for_mobile(scripted_encoder)
    optimized_encoder._save_for_lite_interpreter(path)
    print('successfully saved to ' + path)

def saveDecoderToPTL(decoder, path):
    decoder_input = torch.tensor([[SOS_token]], device=device)
    decoder_hidden = decoder.initHidden()
    encoder_outputs = torch.zeros(MAX_LENGTH, hidden_size, device=device)
    scripted_decoder = torch.jit.trace(
        decoder,
        (decoder_input, decoder_hidden, encoder_outputs)
    )
...
def telephone(sent, times = 10, loop = 50): # starting from english
    dialog = [sent]
    for i in range(times):
        translation = translateEnglishtoFrench(dialog[-1])
        trans_back = translateFrenchtoEnglish(translation)
        j = 0
        while (trans_back == dialog[-1] and j < loop):
            translation = translateEnglishtoFrench(dialog[-1])
            trans_back = translateFrenchtoEnglish(translation)
            j += 1
        dialog.append(translation)
        dialog.append(trans_back)
    return dialog
